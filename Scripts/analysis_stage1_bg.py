...
processList = {
        ...
        #'p8_ee_Zee_ecm91':{'chunks':100}, #number of jobs
        'p8_ee_Zee_ecm91':{'chunks':20},
        'wzp6_ee_gammagamma_ecm91':{'chunks':20},
...
#inputDir = "mydir" + "output_MadgraphPythiaDelphes"

#Optional: output directory, default is local dir
outputDir = "mydir" + "output_stage1"
...
class RDFanalysis():
		...
		#OVERVIEW: Accessing different objects and counting them
              ...
              # Following code is written specifically for the ALP study
                ...
                #The angle between photons with the highest momentum
                .Define("FSGenPhoton_alpha", """
                        if (n_FSGenPhoton >= 2)
                        {
                            size_t max_index1{0}, max_index2{0};
                            double max_val = 0.0;
                            for(size_t count = 0; count < n_FSGenPhoton; ++count)
                            {
                                if (max_val < FSGenPhoton_p.at(count))
                                    {
                                        max_val = FSGenPhoton_p.at(count);
                                        max_index1 = count;
                                    }
                            }
                            max_val = 0.0;
                            ...
                            auto cos_angle = (FSGenPhoton_px.at(max_index1) * FSGenPhoton_px.at(max_index2) +
                            FSGenPhoton_py.at(max_index1) * FSGenPhoton_py.at(max_index2) + 
                            FSGenPhoton_pz.at(max_index1) * FSGenPhoton_pz.at(max_index2) )/
                            (FSGenPhoton_p.at(max_index1) * FSGenPhoton_p.at(max_index2));
                            return cos_angle;
                        }
                """)#UPDATE
                ...
                #photon that was produced directly in Zboson decay
                #UPDATE
                .Define("GenZ_indices", "MCParticle::get_indices(23, {9000005, 22}, true, false, false, true)(Particle, Particle1)")
                #.Define("GenZ", "myUtils::selMC_leg(0)(GenZ_indices, Particle)")
                .Define("GenZPhoton", "myUtils::selMC_leg(2)(GenZ_indices, Particle)")
                .Define("GenZPhoton_e", "MCParticle::get_e( GenZPhoton )")
                .Define("GenZPhoton_p", "MCParticle::get_p( GenZPhoton )")
                .Define("GenZPhoton_pt", "MCParticle::get_pt( GenZPhoton )")
                .Define("GenZPhoton_px", "MCParticle::get_px( GenZPhoton )")
                .Define("GenZPhoton_py", "MCParticle::get_py( GenZPhoton )")
                .Define("GenZPhoton_pz", "MCParticle::get_pz( GenZPhoton )")
                .Define("GenZPhoton_eta", "MCParticle::get_eta( GenZPhoton )")
                .Define("GenZPhoton_theta", "MCParticle::get_theta( GenZPhoton )")
                .Define("GenZPhoton_phi", "MCParticle::get_phi( GenZPhoton )")
                .Define("GenZPhoton_genStatus", "MCParticle::get_genStatus( GenZPhoton )")
                ...
                #Checking the "angle" beetween photons
                .Define("GenALPPhotons_alpha", "return (1 - (GenALP_mass * GenALP_mass)/(2 * abs(GenALPPhoton1_p) * abs(GenALPPhoton1_p)))") #UPDATE
